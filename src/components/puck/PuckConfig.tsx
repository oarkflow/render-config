			...
			fields: {
				content: {
					...
					render: ({ value, onChange }) => (
						<TableItemsField
							value={value || { mode: "manual", dataSource: "manual", headers: ["Column 1", "Column 2"], rows: [["", ""], ["", ""]] }}
							onChange={onChange}
						/>
					),
				},
			...
			fields: {
				...
				items: {
					...
					getItemSummary: (item: { grow?: string; shrink?: string }) =>
						`Flex Item (grow: ${item.grow || "1"}, shrink: ${item.shrink || "0"})`,
					arrayFields: {
						grow: {
							type: "text",
							label: "Flex Grow",
		...
		Grid: {
			fields: {
				...
				items: {
					type: "array",
					label: "Grid Items",
					getItemSummary: (item: { area?: string; column?: string; row?: string }) =>
						`Grid Item ${item.area ? `(area: ${item.area})` : item.column ? `(column: ${item.column})` : ""}`,
					arrayFields: {
						area: {
							type: "text",
							label: "Grid Area",
		...
		Image: {
			fields: {
				image: {
					type: "custom",
					label: "Image",
					render: ({ value, onChange }: { value: ImageProps['image'] | string | undefined, onChange: (value: ImageProps['image']) => void }) => {
						const imageValue = value ? (typeof value === 'object' ? value : { src: value, alt: '' }) : { src: '', alt: '' };
				...
				width: {
					...
					render: ({ value, onChange }) => (
						<SizePicker
							...
							step="1"
							formatValue={(size, unit) => unit === 'auto' ? 'auto' : `${size}${unit}`}
						/>
					),
				},
				height: {
